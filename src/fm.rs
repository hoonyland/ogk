...
impl<'a> FileManager<'a> {
  pub fn new(local_repository: &'a str, remote_repository: &'a str) -> Self {
    ...
    let mut fm = FileManager {
      ...
      _local_repo: None,
      _git_signature: Signature::now("bot-realopen", "hoonyland.newsletter@gmail.com").unwrap(),
    };
  ...
  pub fn make_filename(registration_number: &str, proc_org_name: &str, file_name: &str) -> String {
    let re_illegal_symbols = Regex::new("[\"\n \t()\'~]").unwrap();
    let re_retouch = Regex::new("_+").unwrap();
